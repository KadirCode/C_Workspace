...
7-> Reason i do selection sort twice is for desired form of out output file which is if the counts are the same, the smallest number should appear first.
...
int main(){
 ...
 //Firs Selection Sort for first column.
    ...
    for(int i = 0; i < cursor;i++)
    {
       ...
       {
           if(arr[y][0] < tempNum)      //searcing for minimum number.
           {
               tempNum = arr[y][0];
               tempCount = arr[y][1];
               indexCursor = y;
 ...
 //Second Selection Sort for second column

    tempNum = 0;                //get our first number and its count to temp integers.
    tempCount = 0;
    indexCursor = 0;

    for(int i = 0; i < cursor;i++)      //search for smallest number after our current row.
    {
       tempNum = arr[i][0];
       tempCount = arr[i][1];

       for(int y = i+1; y < cursor; y++)
       {
           if(arr[y][1] < tempCount)    //searcing for minimum count for numbers.
           {
               tempNum = arr[y][0];
               tempCount = arr[y][1];
               indexCursor = y;
