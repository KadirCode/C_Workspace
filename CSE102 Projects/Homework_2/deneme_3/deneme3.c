...
6-> Execute selection sort for second column which are counts of individual numbers. 
...
void selectionSort(int arr[NROWS][NCOLS],int cursor){
        ...
        for(int i = 0; i < cursor;i++)
        {
            ...
            for(int y = i+1; y < cursor; y++)    //search for smallest number after our current row.
            {
                if(column == 0 && arr[y][0] < tempNum)      //searcing for minimum number.
                {
                    tempNum = arr[y][0];
                    tempCount = arr[y][1];
                    indexCursor = y;        //index of the number in array
                }
...
int main(){
    ...
    selectionSort(arr,cursor);   //Selection Sort.
